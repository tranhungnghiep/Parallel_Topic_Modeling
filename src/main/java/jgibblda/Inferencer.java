        ...
        for (newModel.liter = 1; newModel.liter <= niters; newModel.liter++) {
            ...
            for (int m = 0; m < newModel.M; ++m) {
                for (int n = 0; n < newModel.data.docs[m].length; n++) {
                    ...
                    int topic = infSampling(m, n); // sampling, duplicated code to estimator class.
                    // However, newModel is learned with PRIOR from trainModel.
//                    newModel.z[m].set(n, topic);
                    newModel.z[m][n] = (short) topic;
                }
            }//end foreach new doc
...
//        int topic = newModel.z[m].get(n);
    ...
    protected void computeNewTheta() {
        for (int m = 0; m < newModel.M; m++) {
            for (int k = 0; k < newModel.K; k++) {
                newModel.theta[m][k] = (float) ((newModel.nd[m][k] + newModel.alpha) / (newModel.ndsum[m] + newModel.K * newModel.alpha));
            }//end foreach topic
        }//end foreach new document
    }

    /**
     * THNghiep: Seem to have error: repeated newModel.nwsum[k] in denominator.
     */
    protected void computeNewPhi() {
        for (int k = 0; k < newModel.K; k++) {
            for (int _w = 0; _w < newModel.V; _w++) {
                ...
                if (id != null) {
                    newModel.phi[k][_w] = (float) ((trnModel.nw[id][k] + newModel.nw[_w][k] + newModel.beta) / (newModel.nwsum[k] + newModel.nwsum[k] + trnModel.V * newModel.beta));
                }
            }//end foreach word
        }// end foreach topic
    }
